...
"""
绘制 NMF 分析结果的脑区图
为每个 NMF component 生成脑区可视化，显示各个 component 的权重分布
"""
...
NMF_DIR = os.path.join(BASE_DIR, "FILE/test_mean_1.5/MIND_DK318_NMF")
H_MATRIX_FILE = os.path.join(NMF_DIR, "NMF_H_component_roi_weights_long.csv")
TOP_ROIS_FILE = os.path.join(NMF_DIR, "NMF_component_top_ROIs.csv")
...
# Component 配置
COMPONENT_CONFIGS = [
    {"component_name": "Component_1", "display_name": "Component 1: Somatosensory", 
     "short_name": "C1_Somatosensory", "color": "#e74c3c"},
    {"component_name": "Component_2", "display_name": "Component 2: Motor-Frontal", 
     "short_name": "C2_Motor_Frontal", "color": "#3498db"},
    {"component_name": "Component_3", "display_name": "Component 3: Visual-Occipital", 
     "short_name": "C3_Visual_Occipital", "color": "#2ecc71"},
    {"component_name": "Component_4", "display_name": "Component 4: Limbic-Temporal", 
     "short_name": "C4_Limbic_Temporal", "color": "#f39c12"},
]

VIEWS = [("lh", "lateral", "left"), ("rh", "lateral", "right"),
         ("lh", "medial", "left"), ("rh", "medial", "right")]
...
def load_fsaverage_and_annot():
    ...
    return {"lh_mesh": lh_mesh, "rh_mesh": rh_mesh, "lh_sulc": lh_sulc, "rh_sulc": rh_sulc,
            "lh_labels": lh_labels, "rh_labels": rh_labels, "lh_names": lh_names, "rh_names": rh_names}
...
def load_nmf_data():
    """加载 NMF 结果数据"""
    print(f"\nLoading NMF data from: {NMF_DIR}")
    h_df = pd.read_csv(H_MATRIX_FILE)
    top_df = pd.read_csv(TOP_ROIS_FILE)
    components = sorted(h_df['component'].unique())
...
def prepare_component_dataframe(h_df, component_name, weight_threshold=0.1):
    """准备特定 component 的数据"""
...
def build_hemi_maps(comp_data):
    """构建左右半球的 ROI 权重映射"""
    lh_rois = comp_data[comp_data['ROI'].str.startswith('lh_') | comp_data['ROI'].str.startswith('lh.')].copy()
    rh_rois = comp_data[comp_data['ROI'].str.startswith('rh_') | comp_data['ROI'].str.startswith('rh.')].copy()
...
def render_component_views(config, comp_data, fs_data, output_dir, vmax=None):
    ...
    print(f"  Weight range: LH=[{lh_vtx.min():.3f}, {lh_vtx.max():.3f}], "
          f"RH=[{rh_vtx.min():.3f}, {rh_vtx.max():.3f}], vmax={vmax:.3f}")
    ...
    for hemi_tag, view, hemi_name in VIEWS:
        ...
        plotting.plot_surf_stat_map(
            surf_mesh=mesh_by_hemi[hemi_tag], stat_map=vertex_data_by_hemi[hemi_tag],
            hemi=hemi_name, view=view, bg_map=sulc_by_hemi[hemi_tag], bg_on_data=True,
            colorbar=True, symmetric_cbar=False, threshold=0.01, vmin=0.0, vmax=vmax,
            cmap=cmap, axes=axis, darkness=0.55)
        
        fig.suptitle(f"{config['display_name']}\n{hemi_tag.upper()} {view.capitalize()} View", 
                     fontsize=11, y=0.98)
        fig.savefig(out_file, bbox_inches='tight', pad_inches=0.1, dpi=300, facecolor='white')
        plt.close(fig)
        print(f"  Saved: {os.path.basename(out_file)}")
    
    return {'config': config, 'comp_data': comp_data, 'lh_vtx': lh_vtx, 'rh_vtx': rh_vtx, 'vmax': vmax}


def create_summary_figure(component_results, fs_data):
    """创建所有 components 的汇总图"""
    print(f"\n{'='*60}\nCreating summary figure...\n{'='*60}")
    summary_path = os.path.join(OUTPUT_DIR, "All_Components_Summary.png")
    ...
    for row_idx, result in enumerate(component_results):
        ...
        plotting.plot_surf_stat_map(
            surf_mesh=fs_data['lh_mesh'], stat_map=result['lh_vtx'], hemi='left', view='lateral',
            bg_map=fs_data['lh_sulc'], bg_on_data=True, colorbar=True, threshold=0.01,
            vmin=0.0, vmax=vmax, cmap=build_gradient_cmap(config['color']), axes=ax_lh, darkness=0.55)
        ...
        plotting.plot_surf_stat_map(
            surf_mesh=fs_data['rh_mesh'], stat_map=result['rh_vtx'], hemi='right', view='lateral',
            bg_map=fs_data['rh_sulc'], bg_on_data=True, colorbar=True, threshold=0.01,
            vmin=0.0, vmax=vmax, cmap=build_gradient_cmap(config['color']), axes=ax_rh, darkness=0.55)
...
def create_top_rois_barplots(top_df, output_dir):
    """为每个 component 创建 top ROIs 的条形图"""
    print(f"\n{'='*60}\nCreating top ROIs bar plots...\n{'='*60}")
    ...
    for config in COMPONENT_CONFIGS:
        ...
        ax.set_title(f"{config['display_name']}\nTop 20 ROIs by Weight", 
                     fontsize=12, fontweight='bold')
        ax.grid(axis='x', alpha=0.3)
        
        for i, (idx, row) in enumerate(comp_top.iterrows()):
...
def main():
    print("="*60 + "\nNMF Component Brain Visualization\n" + "="*60)
    ...
    for config in COMPONENT_CONFIGS:
        ...
        if comp_name not in components:
            print(f"\nWarning: {comp_name} not found in data, skipping...")
            continue
        
        comp_data = prepare_component_dataframe(h_df, comp_name, weight_threshold=0.05)
    ...
    create_top_rois_barplots(top_df, OUTPUT_DIR)
