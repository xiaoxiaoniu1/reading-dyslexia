...
TMAP_DIR = os.path.join(BASE_DIR, "FILE/test_mean_1.5/Clustering_5")
CLUSTER_FILE = os.path.join(TMAP_DIR, "clustering/degree_Tmap_cluster_K5.csv")
...
CLUSTER_CONFIGS = [
    {
        ...
        "subtitle": "Weak → Strong difference (adult enhancement)",
        "color": "#e74c3c",  # 红色
    },
    {
        "cluster_id": 2,
        "display_name": "Cluster 2: Persistent Moderate",
        "short_name": "C2_Persistent_Moderate",
        "subtitle": "Moderate → Moderate difference (stable pattern)",
        "color": "#3498db",  # 蓝色
    },
    {
        "cluster_id": 3,
        "display_name": "Cluster 3: Developmental Reversal",
        "short_name": "C3_Reversal",
        "subtitle": "Weak/Negative → Positive (reversal pattern)",
        "color": "#2ecc71",  # 绿色
    },
    {
        "cluster_id": 4,
        "display_name": "Cluster 4: Persistent Strong",
        "short_name": "C4_Persistent_Strong",
        "subtitle": "Strong → Moderate/Strong (TD>DD throughout)",
        "color": "#f39c12",  # 橙色
    },
    {
        "cluster_id": 5,
        "display_name": "Cluster 5: Child Predominant",
        "short_name": "C5_Child_Predominant",
        "subtitle": "Moderate/Strong → Weak/Negative (child predominant)",
        "color": "#9b59b6",  # 紫色
    },
]
...
def load_fsaverage_and_annot():
    ...
    # 直接使用已存在的 fsaverage 路径
    fs_dir = os.path.join(BASE_DIR, "FILE", "fsaverage")
...
def build_cluster_description(cluster_rois):
    """生成包含儿童/成人 t 值和 ROI 数量的图示文字"""
    n_rois = len(cluster_rois)
    child_t = cluster_rois['t_child_TD_minus_DD'].mean()
    adult_t = cluster_rois['t_adult_TD_minus_DD'].mean()
    return f"Child: t={child_t:.2f}, Adult: t={adult_t:.2f} (n={n_rois} ROIs)"
...
def render_cluster_views(config, cluster_rois, fs_data, output_dir):
    ...
    for hemi_tag, view, hemi_name in VIEWS:
        ...
        # 添加标题
        description = build_cluster_description(cluster_rois)
        fig.suptitle(
...
def create_summary_figure(cluster_results, fs_data):
    ...
    # 添加总标题
    fig.suptitle(
        'K-means Clustering (K=5) of TD-DD Degree T-maps\nDevelopmental Patterns Across Child and Adult Groups',
        fontsize=14, fontweight='bold', y=0.98
    )
    
    # 创建 5x2 的网格（5个聚类 × 2个半球）
    for row_idx, result in enumerate(cluster_results):
        config = result['config']
        description = build_cluster_description(result['cluster_rois'])
        
        # 左半球 lateral view
        ax_lh = fig.add_subplot(5, 2, row_idx*2 + 1, projection='3d')
        plotting.plot_surf_stat_map(
        ...
        # 右半球 lateral view
        ax_rh = fig.add_subplot(5, 2, row_idx*2 + 2, projection='3d')
        plotting.plot_surf_stat_map(
...
def main():
    print("="*60)
    print("T-map Cluster Brain Visualization (K=5)")
    print("="*60)
