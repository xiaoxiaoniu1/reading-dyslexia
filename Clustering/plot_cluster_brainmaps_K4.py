...
TMAP_DIR = os.path.join(BASE_DIR, "FILE/test_mean_1.5/Clustering_4")
CLUSTER_FILE = os.path.join(TMAP_DIR, "clustering/degree_Tmap_cluster_K4.csv")
...
CLUSTER_CONFIGS = [
    {
        ...
        "subtitle": "Weak → Moderate difference (adult enhancement)",
        "color": "#e74c3c",  # 红色
    },
    {
        "cluster_id": 2,
        "display_name": "Cluster 2: Persistent Strong",
        "short_name": "C2_Persistent_Strong",
        "subtitle": "Moderate → Strong difference (TD>DD throughout)",
        "color": "#3498db",  # 蓝色
    },
    {
        "cluster_id": 3,
        ...
        "subtitle": "TD>DD → DD>TD (reversal pattern)",
        "color": "#2ecc71",  # 绿色
    },
    {
        "cluster_id": 4,
        "display_name": "Cluster 4: Child Predominant",
        "short_name": "C4_Child_Predominant",
        "subtitle": "Strong → Moderate difference (child predominant)",
        "color": "#f39c12",  # 橙色
    },
]
...
def create_summary_figure(cluster_results, fs_data):
    ...
    for row_idx, result in enumerate(cluster_results):
        ...
        # 右半球 lateral view
        ax_rh = fig.add_subplot(4, 2, row_idx*2 + 2, projection='3d')
        plotting.plot_surf_stat_map(
...
def main():
    print("="*60)
    print("T-map Cluster Brain Visualization (K=4)")
    print("="*60)
