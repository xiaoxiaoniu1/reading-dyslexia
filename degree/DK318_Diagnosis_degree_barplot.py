...
# 清理数据
df = df.dropna(subset=['original_project', 'id_old', 'Diagnosis', 'AgeGroup', 'Sex'])
df = df[df['has_file']]
...
for roi in rois_to_plot:
    ...
    output_file = os.path.join(out_dir, f"Significant_Diagnosis_DK318_degree_{roi_safe_name}.png")
...
# 保存绘图数据
plot_data_file = os.path.join(out_dir, "Significant_Diagnosis_DK318_degree_barplot_data.csv")
