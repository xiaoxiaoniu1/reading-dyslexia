...
DEFAULT_OUT_DIR = os.path.join(BASE_DIR, "FILE/DK318-degree")
DEFAULT_ROI_FILE = os.path.join(
    BASE_DIR,
    r"FILE/MIND_t_test/dk318_degree_brainmaps(t-test_Adult_significant)/p_value/significant_regions_p_value.csv"
)
SELECTED_ROIS = ["lh_entorhinal_part1", "rh_parahippocampal_part1"]
...
def load_fsaverage_and_annot():
    ...
    if lh_coords.shape[0] != len(lh_labels) or rh_coords.shape[0] != len(rh_labels):
        raise RuntimeError(
            "Surface vertices do not match annot labels. Ensure fsaverage is used."
        )
...
def plot_selected_rois(selected_rois, out_dir=DEFAULT_OUT_DIR, cmap_name="Reds"):
    ...
    (lh_mesh, rh_mesh, lh_labels, lh_names, rh_labels, rh_names) = load_fsaverage_and_annot()
    plots = [
    ...
    for roi in selected_rois:
        roi_norm = strip_hemi_prefix(roi)
        safe_tag = "".join([c if (c.isalnum() or c in ("_", "-")) else "_" for c in str(roi)])
        if len(safe_tag) == 0:
            safe_tag = "selected"

        roi_map = {roi_norm: 1.0}
        lh_vtx, lh_hit = make_vertex_data(lh_labels, lh_names, roi_map)
        rh_vtx, rh_hit = make_vertex_data(rh_labels, rh_names, roi_map)
        if lh_hit + rh_hit == 0:
            print(f"Warning: ROI not matched annot names: {roi}")
            continue
        ...
        for hemi_tag, view, mesh, hemi_name in plots:
            ...
            display = plotting.plot_surf_stat_map(
                ...
                vmax=vmax,
                cmap=cmap_name,
                title=f"DK318 {roi} | {hemi_tag.upper()} {view}",
                axes=ax,
                darkness=None
            )
...
if __name__ == "__main__":
    ...
    if len(roi_list) == 0 and os.path.exists(DEFAULT_ROI_FILE):
        roi_list.extend(load_rois_from_csv(DEFAULT_ROI_FILE))
