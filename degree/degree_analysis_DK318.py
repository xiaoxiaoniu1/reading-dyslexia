...
def resolve_anova_dir(dataset: str) -> str:
    key = str(dataset).strip().lower()
    mapping = {
        "main": "MIND_DK318_ANOVA_main",
        "sensitivity": "MIND_DK318_ANOVA_sensitivity",
        "sensitive": "MIND_DK318_ANOVA_sensitivity",
    }
    if key not in mapping:
        raise ValueError(f"Unknown dataset: {dataset}. Use one of: {sorted(mapping)}")
    return os.path.join(BASE_DIR, "FILE", "test_mean_1.5", mapping[key])
...
def plot_degree_brainmaps(
    ...
    def infer_alpha_from_col(col_name: str) -> float:
        col_lower = col_name.lower()
        if col_lower.endswith("_0_001"):
            return 0.001
        if col_lower.endswith("_0_01"):
            return 0.01
        return 0.05
    ...
    for VALUE_COL in cols:
        ...
        if VALUE_COL.lower().startswith("p"):
            if is_binary_like(df[VALUE_COL]):
                df_sig = df[df[VALUE_COL].astype(float) > 0.5].copy()
                df_sig[VALUE_COL] = 1.0
                plot_title_prefix = f"{title_prefix} {VALUE_COL} (binary)"
            else:
                alpha = infer_alpha_from_col(VALUE_COL)
                sig_mask = df[VALUE_COL].notna() & (df[VALUE_COL] < alpha)
                df_sig = df[sig_mask].copy()
                base_val = -np.log10(
                    np.clip(
                        df_sig[VALUE_COL].to_numpy(),
                        a_min=np.finfo(float).tiny,
                        a_max=None,
                    )
                )
            ...
            if (not is_binary_like(df[VALUE_COL])) and ("Diagnosis" in VALUE_COL):
                diag_sig_csv = os.path.join(
                    sig_file_dir, "Significant_Diagnosis_DK318_degree_results.csv"
                )
                ...
                try:
                    if os.path.exists(diag_sig_csv):
                        ...
                        if "feature" in df_dir.columns:
                            sig_features = (
                                df_dir["feature"].astype(str).dropna().unique()
                            )

                            df_sig = df[
                                df["feature"].astype(str).isin(sig_features)
                            ].copy()
                        ...
                        if {"mean_TD", "mean_DD"}.issubset(df_dir.columns):
                            ...
                            df_sig = pd.merge(
                                df_sig, df_dir_sub, on="feature", how="left"
                            )
            ...
            # 2. AgeGroup: Adult vs Child (Adult > Child 为正/红)
            ...
            elif (not is_binary_like(df[VALUE_COL])) and ("AgeGroup" in VALUE_COL):
                age_sig_csv = os.path.join(
                    sig_file_dir, "Significant_AgeGroup_DK318_degree_results.csv"
                )
                ...
                if ("mean_Adult" not in df_sig.columns) and os.path.exists(age_sig_csv):
                    ...
                    try:
                        ...
                        if (
                            "mean_Adult" in df_dir.columns
                            and "mean_Child" in df_dir.columns
                        ):
            ...
            elif not is_binary_like(df[VALUE_COL]):
                # 其他情况 (Interaction等)
        ...
        if df_sig.empty:
            if VALUE_COL.lower().startswith("p") and alpha is not None:
                print(f"{VALUE_COL}: 没有显著脑区 (阈值<{alpha})，跳过绘图。")
            else:
                print(f"{VALUE_COL}: 没有显著脑区，跳过绘图。")
        ...
        lh_vtx, lh_roi_names, lh_hit = make_vertex_data(lh_labels, lh_names, lh_map)

        rh_vtx, rh_roi_names, rh_hit = make_vertex_data(rh_labels, rh_names, rh_map)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        default=os.environ.get("DK318_DATASET", "main"),
        choices=["main", "sensitivity", "sensitive"],
    )
    ...
    (lh_mesh, rh_mesh, lh_labels, lh_ctab, lh_names, rh_labels, rh_ctab, rh_names) = (
        load_fsaverage_and_annot()
    )

    # ---- ANOVA 结果 ----

    anova_dir = resolve_anova_dir(args.dataset)
    anova_csv = os.path.join(anova_dir, "ANOVA_DK318_degree_results.csv")

    anova_out = os.path.join(anova_dir, "dk318_degree_brainmaps(ANOVA_significant)")
    ...
    plot_degree_brainmaps(
        ...
        rh_labels=rh_labels,
        rh_names=rh_names,
        title_prefix=f"DK318 ANOVA ({args.dataset})",
    )
