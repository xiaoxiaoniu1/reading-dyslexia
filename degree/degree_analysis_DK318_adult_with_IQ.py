...
def plot_degree_brainmaps(
    ...
    for VALUE_COL in cols:
        ...
        if VALUE_COL.lower().startswith("p"):
            ...
            if "Diagnosis" in VALUE_COL:
                ...
                if os.path.exists(diag_sig_csv):
                    ...
                    try:
                        ...
                        # 仅保留未校正 p 显著的行（若有该列）
                        if "sig_raw" in df_dir.columns:
                            df_dir = df_dir[df_dir["sig_raw"] == True]
                        # 数值：-log10(p_Diagnosis)
                        base_val = -np.log10(df_dir["p_Diagnosis"].astype(float))
                        # 方向：来自 direction 列
                        dir_map = {"TD>DD": 1, "TD<DD": -1, "DD>TD": -1, "DD<TD": 1, "equal": 1}
                        signs = df_dir["direction"].map(dir_map).fillna(1)
                        df_sig = df_dir.copy()
                        df_sig[VALUE_COL] = base_val * signs
                        # 回填 roi_norm 供后续映射
                        df_sig["roi_norm"] = df_sig["feature"].astype(str).map(strip_hemi_prefix)
                        plot_title_prefix = f"{title_prefix} Signed -log10(p_Diagnosis)\n(Red: TD>DD, Blue: DD>TD)"
                        use_symmetric_cbar = True
                        cmap_name = 'coolwarm'
                        is_signed = True
                    except Exception as e:
                        print(f"Warning: Failed to use significant diagnosis table: {e}")
            ...
            # 3. IQ: correlation direction (IQ+ / IQ-)
            elif "IQ" in VALUE_COL:
                iq_sig_csv = os.path.join(sig_file_dir, "Significant_IQ_DK318_degree_results.csv")

                if ("corr_IQ" not in df_sig.columns) and os.path.exists(iq_sig_csv):
                    print(f"Loading directional info from: {iq_sig_csv}")
                    try:
                        df_dir = pd.read_csv(iq_sig_csv)
                        if "corr_IQ" in df_dir.columns:
                            df_dir_sub = df_dir[["feature", "corr_IQ"]]
                            df_sig = pd.merge(df_sig, df_dir_sub, on="feature", how="left")
                    except Exception as e:
                        print(f"Warning: Failed to load direction file: {e}")

                if "corr_IQ" in df_sig.columns:
                    diff = df_sig["corr_IQ"]
                    signs = np.sign(diff)
                    signs[diff == 0] = 1
                    signs = signs.fillna(1)
...
if __name__ == "__main__":
    ...
    anova_csv = os.path.join(
        BASE_DIR, r"FILE/MIND_ANOVA_adult_with_IQ", "ANOVA_DK318_degree_results.csv"
    )
    anova_out = os.path.join(BASE_DIR, "FILE/MIND_ANOVA_adult_with_IQ/dk318_degree_brainmaps(ANOVA_significant)")
    anova_cols = [
        "p_Diagnosis_FDR",
        "p_IQ_FDR",
        "p_Diagnosis",
    ...
    plot_degree_brainmaps(
        ...
        rh_labels=rh_labels,
        rh_names=rh_names,
        title_prefix="DK318 ANCOVA (Adult+IQ)",
    )
