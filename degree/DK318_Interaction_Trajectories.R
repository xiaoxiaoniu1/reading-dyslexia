# ==============================================================================
# GAMLSS 组间交互作用显著脑区的发育轨迹分析 (DK318 版)
...
mind_csv_dir <- file.path(base_dir, "fs_subjects_all/MIND_out_combat_degree/")
anova_file   <- file.path(base_dir, "FILE/MIND_ANOVA/ANOVA_DK318_degree_results.csv")
plot_out_dir <- file.path(base_dir, "FILE/MIND_ANOVA/Growth_Trajectories_Interaction")
...
df_demo <- read_excel(demo_file, sheet = "Sheet1") %>%
  mutate(
    ...
    # DK318 文件命名规则: AdoleC_2101_degree.csv
    file_name = paste0(`original-project`, "_", id_old, "_degree.csv"),
...
for (i in 1:nrow(df_demo)) {
  ...
  # 提取显著脑区 (DK318 degree 文件列名为 region)
  tmp_sig <- tmp %>% filter(region %in% sig_rois)
  if (nrow(tmp_sig) > 0) {
    # 合并人口学信息
    tmp_sig$id <- df_demo$id_old[i]
