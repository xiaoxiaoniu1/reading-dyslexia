#!/bin/bash
#SBATCH --job-name=DK318_DGLM_main_iq
#SBATCH --output=analysis/logs/DK318_DGLM_main_iq_%j.out
#SBATCH --error=analysis/logs/DK318_DGLM_main_iq_%j.err
...
Rscript dglm_dk318_degree_edge_main_iq.R "${ANALYSIS_TYPE}"
...
if [[ "${ANALYSIS_TYPE}" == "both" || "${ANALYSIS_TYPE}" == "degree" ]]; then
  ...
  bash /data/home/tqi/data1/share/after_freesurfer/CODE/analysis/run_violin_dk318.sh iq
  echo "===== STEP 3/3 DONE ====="
  echo "Output dir (STEP 3/3): ${VIOLIN_DIR}"
fi
