...
out_edge_csv   <- file.path(out_dir, "GAM_DK318_edge_results.csv")
...
df <- df %>%
  ...
  filter(!is.na(original_project), !is.na(id_old), !is.na(Age_Raw), has_file == TRUE)
...
output_sig_gam_main_effect <- function(res_df, feature_mat, df_demo, effect_name, feature_type, out_dir) {
  ...
  out_file <- file.path(out_dir, paste0("Significant_", effect_name, "_GAM_DK318_", feature_type, "_results.csv"))
