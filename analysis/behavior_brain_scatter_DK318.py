...
mind_dir_csv = "/data/home/tqi/data1/share/after_freesurfer/fs_subjects_all/MIND_out_combat_degree"
mind_dir_npy = "/data/home/tqi/data1/share/after_freesurfer/fs_subjects_all/MIND_out_combat(npy)"

output_dir = "/data/home/tqi/data1/share/after_freesurfer/FILE/behavior_brain_scatter"
...
def resolve_mind_file(row, project_col, id_col):
    if project_col and id_col:
        project = str(row[project_col]).strip()
        sid = str(row[id_col]).strip()
        if project != "nan" and sid != "nan":
            base = f"{project}_{sid}"
            if use_labeled_csv:
                f = os.path.join(mind_dir_csv, f"{base}_MIND_DK318_combat_labeled.csv")
                if os.path.exists(f):
                    return f
                hits = glob.glob(os.path.join(mind_dir_csv, f"*_{sid}_MIND_DK318_combat_labeled.csv"))
                if len(hits) == 1:
                    return hits[0]
            else:
                f = os.path.join(mind_dir_npy, f"{base}_MIND_DK318_combat.npy")
                if os.path.exists(f):
                    return f
                hits = glob.glob(os.path.join(mind_dir_npy, f"*_{sid}_MIND_DK318_combat.npy"))
                if len(hits) == 1:
                    return hits[0]
    return np.nan
...
def get_roi_names_from_any_labeled():
    files = glob.glob(os.path.join(mind_dir_csv, "*_MIND_DK318_combat_labeled.csv"))
    if not files:
        return None
...
def main():
    ...
    project_col = find_col(df, ["original-project", "original_project", "project", "dataset", "site_project", "study"])
    id_col = find_col(df, ["id_old", "id", "subject_id", "sub_id", "sid", "participant_id"])
    ...
    df["mind_file"] = df.apply(lambda r: resolve_mind_file(r, project_col, id_col), axis=1)
    df = df[df["mind_file"].notna()].copy()
    df["mind_file"] = df["mind_file"].astype(str)
    df = df[df["mind_file"].apply(os.path.exists)].copy()

    if df.empty:
        raise RuntimeError("No valid subjects matched to MIND files. Please check ID columns.")
