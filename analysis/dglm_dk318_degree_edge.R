...
out_dir <- "/data/home/tqi/data1/share/after_freesurfer/FILE/test_any_2/MIND_DGLM_DK318/"
...
deg_mean_interaction_unc_sig <- degree_res[
  !is.na(degree_res$p_mean_Interaction) & degree_res$p_mean_Interaction < threshold, ]
deg_disp_interaction_fdr_sig <- degree_res[
  !is.na(degree_res$p_disp_Interaction_FDR) & degree_res$p_disp_Interaction_FDR < threshold, ]
deg_disp_interaction_unc_sig <- degree_res[
  !is.na(degree_res$p_disp_Interaction) & degree_res$p_disp_Interaction < threshold, ]

write.csv(deg_mean_interaction_fdr_sig, out_deg_mean_interaction_fdr_csv, row.names = FALSE)
write.csv(deg_mean_interaction_unc_sig, out_deg_mean_interaction_unc_csv, row.names = FALSE)
write.csv(deg_disp_interaction_fdr_sig, out_deg_disp_interaction_fdr_csv, row.names = FALSE)
write.csv(deg_disp_interaction_unc_sig, out_deg_disp_interaction_unc_csv, row.names = FALSE)
cat(sprintf("\nDegree mean interaction significant (FDR): %d ROIs. Saved: %s\n",
            nrow(deg_mean_interaction_fdr_sig), out_deg_mean_interaction_fdr_csv))
cat(sprintf("Degree mean interaction significant (uncorrected): %d ROIs. Saved: %s\n",
            nrow(deg_mean_interaction_unc_sig), out_deg_mean_interaction_unc_csv))
cat(sprintf("Degree dispersion interaction significant (FDR): %d ROIs. Saved: %s\n",
            nrow(deg_disp_interaction_fdr_sig), out_deg_disp_interaction_fdr_csv))
cat(sprintf("Degree dispersion interaction significant (uncorrected): %d ROIs. Saved: %s\n",
            nrow(deg_disp_interaction_unc_sig), out_deg_disp_interaction_unc_csv))
...
edge_mean_interaction_unc_sig <- edge_res[
  !is.na(edge_res$p_mean_Interaction) & edge_res$p_mean_Interaction < threshold, ]
edge_disp_interaction_fdr_sig <- edge_res[
  !is.na(edge_res$p_disp_Interaction_FDR) & edge_res$p_disp_Interaction_FDR < threshold, ]
edge_disp_interaction_unc_sig <- edge_res[
  !is.na(edge_res$p_disp_Interaction) & edge_res$p_disp_Interaction < threshold, ]

write.csv(edge_mean_interaction_fdr_sig, out_edge_mean_interaction_fdr_csv, row.names = FALSE)
write.csv(edge_mean_interaction_unc_sig, out_edge_mean_interaction_unc_csv, row.names = FALSE)
write.csv(edge_disp_interaction_fdr_sig, out_edge_disp_interaction_fdr_csv, row.names = FALSE)
write.csv(edge_disp_interaction_unc_sig, out_edge_disp_interaction_unc_csv, row.names = FALSE)
cat(sprintf("\nEdge mean interaction significant (FDR): %d edges. Saved: %s\n",
            nrow(edge_mean_interaction_fdr_sig), out_edge_mean_interaction_fdr_csv))
cat(sprintf("Edge mean interaction significant (uncorrected): %d edges. Saved: %s\n",
            nrow(edge_mean_interaction_unc_sig), out_edge_mean_interaction_unc_csv))
cat(sprintf("Edge dispersion interaction significant (FDR): %d edges. Saved: %s\n",
            nrow(edge_disp_interaction_fdr_sig), out_edge_disp_interaction_fdr_csv))
cat(sprintf("Edge dispersion interaction significant (uncorrected): %d edges. Saved: %s\n",
            nrow(edge_disp_interaction_unc_sig), out_edge_disp_interaction_unc_csv))
