...
# Covariates: Site, Sex
# 使用原始 MIND 矩阵（不经过 combat），在模型中控制 Site
# ============================================================
...
mind_csv_dir <- "/data/home/tqi/data1/share/after_freesurfer/fs_subjects_all/MIND(DK68)"

out_dir <- "/data/home/tqi/data1/share/after_freesurfer/FILE/MIND_DK68_ANOVA_with_site/"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

out_degree_csv <- file.path(out_dir, "ANOVA_DK68_degree_results_with_site.csv")
out_edge_csv   <- file.path(out_dir, "ANOVA_DK68_edge_results_with_site.csv")
...
# ============================================================
# 6) ROI-wise two-way ANOVA (degree) - 加入 Site 作为协变量
# ============================================================
run_two_way_anova_per_feature <- function(y_mat, df_sub, feature_names,
  ...
  res <- lapply(seq_len(ncol(y_mat)), function(k) {
    ...
    # 正常拟合 - 加入 Site 作为协变量
    fit <- lm(y2 ~ Diagnosis * AgeGroup + Sex + Site, data = d2)
...
cat("\n=== Running ROI-wise ANOVA (degree) with Site as covariate ===\n")
...
# ============================================================
# 7) Edge-wise two-way ANOVA (upper-triangle edges) - 加入 Site 作为协变量
...
cat("\n=== Running Edge-wise ANOVA (upper triangle) with Site as covariate ===\n")
...
output_sig_results <- function(res_df, feature_mat, df_demo, effect_name, 
  ...
  out_file <- file.path(out_dir, 
                        paste0("Significant_", effect_name, "_DK68_", 
                              feature_type, "_results_with_site.csv"))
