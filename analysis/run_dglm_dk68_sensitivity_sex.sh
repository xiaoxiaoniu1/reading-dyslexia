#!/bin/bash
#SBATCH --job-name=DK68_DGLM_sensitivity_sex
#SBATCH --output=analysis/logs/DK68_DGLM_sensitivity_sex_%j.out
#SBATCH --error=analysis/logs/DK68_DGLM_sensitivity_sex_%j.err
...
Rscript dglm_dk68_degree_edge_sensitivity_sex.R "${ANALYSIS_TYPE}"
echo "===== STEP 1/3 DONE ====="
if [[ "${ANALYSIS_TYPE}" == "both" || "${ANALYSIS_TYPE}" == "degree" ]]; then
  ...
  python /data/home/tqi/data1/share/after_freesurfer/CODE/degree/degree_analysis_DK68_DGLM.py --result-dir "${RESULT_DIR}" --stratified-dir "${RESULT_DIR}"
  echo "===== STEP 2/3 DONE ====="
  echo "===== STEP 3/3: Run VIOLIN follow-up plots ====="
  echo "Output dir (STEP 3/3): ${VIOLIN_DIR}"
  bash /data/home/tqi/data1/share/after_freesurfer/CODE/analysis/run_violin_dk68.sh sensitivity_sex
  echo "===== STEP 3/3 DONE ====="
fi
echo "End: $(date)"
