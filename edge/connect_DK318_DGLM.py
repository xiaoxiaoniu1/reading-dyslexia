import pandas as pd
import numpy as np
from nilearn import plotting
import matplotlib.pyplot as plt
import matplotlib.colors as mcolors
import os

# ==========================================
# 1. 读取坐标文件
# ==========================================
coords_file = '/data/home/tqi/data1/share/after_freesurfer/FILE/DK-318/DK318_MNI_Coordinates.csv'
coords_df = pd.read_csv(coords_file)
node_coords = coords_df[['x', 'y', 'z']].values
roi_names = coords_df['label'].values  # 保存脑区名称

n_nodes = len(node_coords)
print(f"加载了 {n_nodes} 个脑区坐标")

# ==========================================
# 2. 读取 DGLM 结果并转换为矩阵
# ==========================================
dglm_file = '/data/home/tqi/data1/share/after_freesurfer/FILE/test_mean_1.5/MIND_DK318_DGLM/DGLM_DK318_edge_results.csv'
edge_df = pd.read_csv(dglm_file)

# 读取 combat 后的矩阵数据用于计算效应方向
demo_file = '/data/home/tqi/data1/share/after_freesurfer/FILE/test_mean_1.5/all_data_cqt_mean_1.5.xlsx'
demo_df = pd.read_excel(demo_file)
mind_dir = '/data/home/tqi/data1/share/after_freesurfer/FILE/test_mean_1.5/MIND_DK318_combat/'

# ==========================================
# 3. 计算每条边的效应方向
# ==========================================
def compute_edge_effect_direction(demo_df, mind_dir, n_nodes, group_col, group_a, group_b):
    demo_df = demo_df.copy()
    demo_df['subj_prefix'] = demo_df['original-project'].astype(str) + '_' + demo_df['id_old'].astype(str) + '_MIND_DK318'
    demo_df['mind_file'] = mind_dir + demo_df['subj_prefix'] + '_combat.csv'
    demo_df['exists'] = demo_df['mind_file'].apply(os.path.exists)
    demo_df = demo_df[demo_df['exists']].copy()

    all_matrices = []
    groups = []

    for _, row in demo_df.iterrows():
        try:
            mat = pd.read_csv(row['mind_file'], index_col=0).values
            if mat.shape == (n_nodes, n_nodes):
                all_matrices.append(mat)
                groups.append(row[group_col])
        except:
            continue

    all_matrices = np.array(all_matrices)
    groups = np.array(groups)

    a_mean = all_matrices[groups == group_a].mean(axis=0)
    b_mean = all_matrices[groups == group_b].mean(axis=0)

    diff_matrix = a_mean - b_mean

    return diff_matrix

print("正在计算效应方向（TD vs DD / Adult vs Child）...")
effect_direction_diagnosis = compute_edge_effect_direction(demo_df, mind_dir, n_nodes, "group_d_or_c", 0, 1)
effect_direction_age = compute_edge_effect_direction(demo_df, mind_dir, n_nodes, "group_age", 1, 2)
print("效应方向计算完成！")

# ==========================================
# 4. 配置绘图参数
# ==========================================
p_threshold = 0.05 
top_n_edges = 20  # 只画显著性最高的前20条边

# 定义要分析的列：Mean 和 Dispersion 的交互（uncorrected 和 FDR）
target_p_cols = [
    'p_mean_Interaction',
    'p_mean_Interaction_FDR',
    'p_disp_Interaction',
    'p_disp_Interaction_FDR'
]

# 输出目录
output_dir = '/data/home/tqi/data1/share/after_freesurfer/FILE/test_mean_1.5/MIND_DK318_DGLM/'
os.makedirs(output_dir, exist_ok=True)

# ==========================================
# 5. 对每一列分别画图
# ==========================================
for target_p_col in target_p_cols:
    print(f"\n{'='*50}")
    print(f"处理: {target_p_col}")
    print(f"{'='*50}")
    
    # 筛选显著边
    significant_edges = edge_df[edge_df[target_p_col] < p_threshold].copy()
    if len(significant_edges) == 0:
        print(f"⚠️ 提示：{target_p_col} 无显著结果 (p < {p_threshold})")
        continue
    
    print(f"找到 {len(significant_edges)} 条显著边")
    
    significant_edges = significant_edges.sort_values(by=target_p_col).head(top_n_edges)
    print(f"保留显著性最高的前 {top_n_edges} 条边")
    
    # 对于交互项，使用诊断效应方向
    effect_direction_matrix = effect_direction_diagnosis
    pos_label = 'TD'
    neg_label = 'DD'

    matrix = np.zeros((n_nodes, n_nodes))
    for _, row in significant_edges.iterrows():
        i, j = int(row['i']) - 1, int(row['j']) - 1
        
        p_val = row[target_p_col]
        effect_sign = np.sign(effect_direction_matrix[i, j])
        strength = -np.log10(p_val) * effect_sign
        
        matrix[i, j] = matrix[j, i] = strength
    
    print(f"矩阵构建完成！")
    print(f"  {pos_label}更强（正值）: {np.sum(matrix > 0) // 2} 条")
    print(f"  {neg_label}更强（负值）: {np.sum(matrix < 0) // 2} 条")
    
    # 保存显著边表格
    edge_table = significant_edges.copy()
    edge_table['ROI_i_name'] = edge_table['i'].apply(lambda x: roi_names[int(x)-1])
    edge_table['ROI_j_name'] = edge_table['j'].apply(lambda x: roi_names[int(x)-1])
    edge_table['effect_direction'] = edge_table.apply(
        lambda r: effect_direction_matrix[int(r['i'])-1, int(r['j'])-1],
        axis=1
    )
    edge_table['stronger_in'] = edge_table['effect_direction'].apply(
        lambda x: pos_label if x > 0 else neg_label
    )
    
    edge_table_path = f"{output_dir}DGLM_DK318_{target_p_col}_top{top_n_edges}_edges.csv"
    edge_table[['ROI_i_name', 'ROI_j_name', 'i', 'j', target_p_col, 'effect_direction', 'stronger_in']].to_csv(edge_table_path, index=False)
    print(f"✓ 显著边表格已保存: {edge_table_path}")
    
    # 绘图
    max_abs = max(abs(matrix.min()), abs(matrix.max()), 1e-6)
    
    fig = plotting.plot_connectome(
        adjacency_matrix=matrix,
        node_coords=node_coords,
        display_mode='lyrz',
        node_size=1,
        node_color='black',
        edge_cmap='RdBu_r',
        edge_vmin=-max_abs,
        edge_vmax=max_abs,
        edge_threshold=0,  # 显示所有非零边
        edge_kwargs={'linewidth': 1.5},
        colorbar=True,
        title=f"DK318 {target_p_col} -log10(p) (Red:{pos_label}>{neg_label}, Blue:{neg_label}>{pos_label})"
    )
    
    output_path = f"{output_dir}DGLM_DK318_{target_p_col}_plot.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ 图片已保存: {output_path}")

print(f"\n{'='*50}")
print("所有 DK318 DGLM edge 图片生成完成！")
print(f"{'='*50}")
